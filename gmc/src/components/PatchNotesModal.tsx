...
const PATCH_NOTES: PatchNoteEntry[] = [
  {
    ...
    items: [
      ...
      '학교 사이트가 일시적으로 로그인에 실패해도 최대 2회까지 자동으로 재시도한 뒤에만 비밀번호 재입력을 요청하도록 개선했습니다.',
      '개발자 도구에 GMCAuto API 사용 가이드 페이지가 추가되어, API 키 목록 화면에서 바로 인증 방식·엔드포인트·스코프·오류 코드를 확인할 수 있습니다.',
      '좌측 메뉴의 "PASS 신청"이 "PASS 수동 신청"으로 이름이 바뀌었습니다. 자동 신청 등록과 헷갈리지 않도록 구분했습니다.',
      '중단 기간(방학 등) 중에 금요일·주말·공휴일마다 안내 알림이 매번 반복해서 오던 문제를 수정했습니다. 이제 중단 기간 시작 시 한 번만 안내됩니다.',
      'GMCAuto 공개 API 응답 스키마가 1.1로 갱신되었습니다(개발자 도구 → GMCAuto API 사용 가이드 참고). hasApplied의 의미가 "자동 신청 등록 여부"로 바뀌었고, 오늘 성공 여부(isSucceededToday)와 지정 날짜 성공 여부(isSucceeded, ?date= 필요)가 새로 추가되었습니다. 기존 API 사용처가 없어 하위 호환 없이 적용되었습니다.',
      '로그인 버튼이 "학교 Google 계정으로 로그인"으로 바뀌고 아이콘도 Google 로고로 변경되었습니다. 실제 로그인 방식(Akademiya 경유)은 그대로이며, 버튼 아래 작은 안내 문구로 계속 표시됩니다.',
    ],
  },
  {
    version: 2,
