...
export default function TermsOfUseModal({ sessionId, consentedVersion, onConsented }: TermsOfUseModalProps) {
  const { t } = useTranslation()
  const [agreed, setAgreed] = useState(false)
  const [loading, setLoading] = useState(false)
  const [error, setError] = useState('')
  const [showFull, setShowFull] = useState(false)
  const terms = gmcTermsOfUse

  // 마지막 동의 버전 이후의 변경 요약만 누적 표시 (예: v1 동의자 → v1→v2 ...)
  const newSummaries = gmcTermsSummaries.filter((s) => s.to > consentedVersion)
    ...
    <div style={styles.overlay}>
      <div style={styles.modal}>
        <div style={styles.header}>
          <h2 style={styles.title}>{terms.title}</h2>
          <p style={styles.subtitle}>
            {consentedVersion > 0
              ? '이용약관이 개정되었습니다. 변경사항을 확인하고 다시 동의해 주세요.'
              : '서비스 이용을 위해 이용약관에 동의해 주세요.'}
          </p>
          <span style={styles.version}>v{terms.version} · {terms.effectiveDate}</span>
        </div>
        ...
        <div style={styles.content}>
          {showSummary ? (
            ...
            <>
              <p style={styles.preamble}>{terms.preamble}</p>
