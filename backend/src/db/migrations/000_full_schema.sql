...
-- ============================================================
-- (통합됨: 원본 009_ai_conversations.sql / 010_ai_vault.sql — 이후 완전 제거됨)
-- ============================================================
-- AkashaAlt(ai.akademiya.kr)는 이제 Akademiya 본체와 DB/백엔드를 공유하지 않는
-- 완전히 독립된 앱(GMCAuto와 동일한 구조)이다. 채팅 기록·API Key Vault 테이블은
-- 전부 별도 akashaalt 스키마(akashaalt/server/db.ts)로 이전되었다.
-- ⚠️ 기존 프로덕션 DB에 이미 생성된 ai_conversations/ai_messages/ai_vaults/
-- ai_api_keys/ai_vault_reset_tokens 데이터는 여기서 자동으로 지워지지 않는다 —
-- akashaalt DB로 수동 이관 후 DROP TABLE 해야 한다 (DEPLOYMENT.md 참조).
